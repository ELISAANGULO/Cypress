...
class PublicarPost {
    publicarPost() {
        ...
        for(let i=0; i< 5; i++){
            cy.wait(1000)
            cy.get('a[class="ember-view"]').contains('Posts').click({force: true})
            cy.wait(3000)
            ...
            cy.wait(2000)
            cy.get('a[class="ember-view permalink gh-list-data gh-post-list-title"]').contains('h3','(Untitled)').should('not.be.empty')
            cy.wait(2000)
